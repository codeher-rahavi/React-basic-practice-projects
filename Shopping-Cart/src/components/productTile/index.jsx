...
function ProductTile({ singleProductTile }) {
    ...
    return (
        <div className="relative group border border-cyan-700 p-6 cursor-pointer">
            ...
            <button onClick={()=> handleNavigateToProductDetailsPage(singleProductTile?.id)} className="px-5 mt-5 w-full py-2 rounded-none bg-black text-white font-bold text-lg">
                View Details
            </button>
            <button
            disabled = {cartItems.findIndex(item => item.id === singleProductTile?.id) > -1}
             onClick={() => handleAddToCart(singleProductTile)} className=" disabled:opacity-65 px-5 mt-5 w-full py-2 rounded-none bg-black text-white font-bold text-lg">Add To Cart</button>
        </div>
